...
logger = setup_logging(
    "llamasearch.ui.main", use_qt_handler=True
)  # Initial setup for early logs
...
class MainWindow(QMainWindow):
    ...
    def closeEvent(self, event):
        ...
        try:
            ...
            logger.error(
                f"Error during executor shutdown: {exec_shutdown_err}", exc_info=True
            )
        ...
        try:
            ...
            if sys.version_info >= (3, 7):
                try:
                    ...
                    logger.debug(
                        "RuntimeError getting asyncio tasks (no loop likely set)."
                    )
                    pass
            ...
            if not all_loops:  # Fallback if no tasks found / older python
                ...
                try:
                    ...
                    logger.debug(
                        "RuntimeError getting default event loop (no loop likely set)."
                    )
                    pass  # No default loop running

            if all_loops:
                logger.warning(
                    f"Found {len(all_loops)} potentially running asyncio loops during final shutdown. Attempting cleanup."
                )
            ...
            logger.error(
                f"Error during final asyncio cleanup check: {ae}", exc_info=True
            )
...
def main():
    ...
    logger.info(
        f"--- Starting LlamaSearch GUI (Version: {QApplication.applicationVersion()}) ---"
    )
    ...
    try:
        ...
        logger.error(
            f"Could not register SIGINT handler: {sig_err}. Ctrl+C might not work gracefully."
        )

    exit_code = 0
    if (
        _app_instance
    ):  # Guard against _app_instance being None if init failed very early
    ...
    if _main_window and hasattr(_main_window, "_executor") and _main_window._executor and not _main_window._executor._shutdown:  # type: ignore
        logger.warning(
            "Executor was not shut down before application exit. Forcing quick shutdown."
        )
