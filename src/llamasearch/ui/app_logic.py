...
class LlamaSearchApp:
    ...
    def _initialize_llm_search(self) -> bool:
        ...
        try:
            ...
            if self.llm_search and self.llm_search.model:
                ...
                logger.error(
                    "LLMSearch initialization succeeded, but LLM component failed."
                )
                if self.llm_search:
            ...
            def _intermediate_callback(f):
                ...
                try:
                    if f.cancelled():
                        ...
                        if exception:
                            logger.debug(
                                f"Future completed with exception: {type(exception).__name__}"
                            )
                        else:
                            # Get result only if no exception
                            result = f.result()
                            logger.debug(
                                f"Future completed with result type: {type(result)}"
                            )
                ...
                logger.debug(
                    "Emitting internal task completed signal for final GUI update."
                )
    ...
    def _final_gui_callback(
        ...
        logger.debug(
            f">>> GUI Callback: Result={type(result)}, Exc={type(exception)}, Cancel={cancelled}"
        )
        ...
        try:
            ...
            if exception:
                ...
                if not self._shutdown_event.is_set():
                    ...
                    )  # Avoid huge tracebacks in UI logs
            ...
            if result is not None:  # Added check for None before tuple check
                ...
                if is_expected_tuple:
                    ...
                    logger.debug(
                        f"Emitting completion signal: Success={success}, Msg='{result_message_str[:100]}...'"
                    )
                ...
                logger.warning(
                    "Task completed with None result and no exception/cancellation."
                )
            ...
            try:
                ...
                logger.critical(
                    f"Failed even to emit error signal after callback failure: {emit_err}"
                )
    ...
    def submit_removal(self, source_identifier_to_remove: str):
        ...
        if not self.llm_search:
            ...
            return
        if (
            not isinstance(source_identifier_to_remove, str)
            or not source_identifier_to_remove
        ):
            # Use attribute access for emit
        ...
        logger.info(
            f"Submitting removal task for source identifier: {source_identifier_to_remove}"
        )
        try:
            ...
            display_name = (
                (display_name[:40] + "...") if len(display_name) > 40 else display_name
            )
        except Exception:
    ...
    def _execute_crawl_and_index_task(
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Crawl phase OK ({crawl_duration:.1f}s). Collected {url_count} URLs."
                )
                ...
                if crawl_start_time > 0:
                    crawl_duration = time.time() - crawl_start_time
                logger.error(
                    f"Crawl FAILED ({crawl_duration:.1f}s): {crawl_exc}",
                    exc_info=self.debug,
                )
            ...
            if crawl_successful and collected_urls is not None:
                ...
                if self.llm_search and hasattr(self.llm_search, "_load_reverse_lookup"):
                    try:
                        logger.info(
                            "Refreshing reverse lookup cache in LLMSearch instance..."
                        )
                        self.llm_search._load_reverse_lookup()
                    except Exception as lookup_reload_err:
                        logger.error(
                            f"Failed to reload reverse lookup before indexing: {lookup_reload_err}",
                            exc_info=self.debug,
                        )
                # --- END MODIFICATION ---
                ...
                try:
                    ...
                    if raw_output_dir.is_dir():
                        ...
                        for md_file in files_to_index:
                            ...
                            added_count, blocked = self.llm_search.add_source(
                    ...
                    logger.info(
                        f"File processing phase completed ({index_duration:.1f}s). Indexed {processed_files_count} files. Total chunks added: {total_added_chunks}."
                    )

                    if self._shutdown_event.is_set():
                        raise InterruptedError(
                            "Shutdown detected after indexing phase."
                        )
                    ...
                    if indexing_start_time > 0:
                        index_duration = time.time() - indexing_start_time
                    logger.error(
                        f"Indexing FAILED ({index_duration:.1f}s): {index_exc}",
                        exc_info=self.debug,
                    )
                    final_message = (
                        f"Indexing failed: {index_exc}"  # Set specific error
                    )
                ...
                logger.warning(
                    "Crawl reported success but returned no URL list. Skipping indexing."
                )
    ...
    def _execute_manual_index_task(self, path_str: str) -> Tuple[str, bool]:
        ...
        try:
            ...
            if self._shutdown_event.is_set():
                ...
                raise InterruptedError(
                    "Shutdown detected after manual index processing."
                )
            ...
            if was_blocked:
                ...
                added_count >= 0
            ):  # 0 chunks added is still success if not blocked (e.g., unchanged file)
                total_added_chunks = added_count
                logger.info(
            ...
            task_successful = False
            final_message = (
                f"Indexing '{source_path.name}' interrupted."  # Set specific message
            )
        except Exception as e:
            logger.error(
    ...
    def _execute_removal_task(
        self, source_identifier_to_remove: str
    ) -> Tuple[str, bool]:
        ...
        try:
            logger.debug(
                f"Executing removal task for identifier: {source_identifier_to_remove}"
            )
            if self._shutdown_event.is_set():
            ...
            task_successful = (
                True  # If no exception, consider the task logic successful
            )
            ...
            task_successful = False
            final_message = (
                f"Removal of '{display_name}' interrupted."  # Set specific message
            )
        except Exception as e:
            logger.error(
            ...
            task_successful = False  # Ensure false on exception
            final_message = (
                f"Error removing '{display_name}'. See logs."  # Set specific message
            )
    ...
    def apply_settings(self, settings: Dict[str, Any]):
        ...
        new_max_results_val = settings.get(
            "max_results", self._current_config.get("max_results", 3)
        )
        try:
            ...
            if new_max_results > 0 and new_max_results != current_max:
                ...
                logger.warning(
                    f"Invalid Max Results value ignored: {new_max_results}. Must be > 0."
                )
