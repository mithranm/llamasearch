...
def load_nlp_model_for_lang(lang: str = "en") -> spacy.language.Language:
    ...
    For tokenization, we attempt a transformer model and fall back if necessary.
    """
    lang = lang.lower()
    model_name = ""
    if lang == "en":
        ...
        fallback_model = "ru_core_news_sm"
    else:
...
def improved_tokenizer(text: str, nlp: spacy.language.Language) -> List[str]:
    ...
    This ensures consistency with the KnowledgeGraph tokenization.
    """
    doc = nlp(text)
    tokens = []
    for token in doc:
...
class BM25Retriever:
    ...
    This version is multilingual and uses an improved tokenizer.
    """
    ...
    def _load_or_init_index(self) -> None:
        """
        Loads existing index metadata from disk, or initializes empty lists.
        """
    ...
    def _build_entity_indices(self) -> None:
        """
        Build a term-to-document index from the tokenized corpus.
        """
        self.term_indices = {}
    ...
    def save(self) -> None:
        """
        Saves the index metadata (documents, doc_ids, tokenized_corpus) to disk.
        """
        data = {
    ...
    def query(self, query_text: str, n_results: int = 5) -> Dict[str, Any]:
        """
        Performs a BM25 query on the index and returns top results.
        """
        ...
        # Boost for documents containing proper nouns in query.
        # Increased boost factor from 1.5 to 2.0 to improve matching for romanized multilingual entities.
