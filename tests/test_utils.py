...
class TestGetLlamasearchDir(unittest.TestCase):
    def test_dir_from_data_manager(self, MockPath, mock_data_manager_instance):
        ...
        mock_data_manager_instance.get_data_paths.return_value = {
            "base": mock_base_path_str
        }
    ...
    def test_dir_fallback_to_home(self, MockPath, mock_data_manager_instance):
        ...
        mock_fallback_dir_instance.mkdir.assert_called_once_with(
            parents=True, exist_ok=True
        )
        self.assertIs(result_path, mock_fallback_dir_instance)

        # Verify that Path() was not called with a string (which would happen if Path.home() was not used for fallback)
        path_constructor_calls = [
            c for c in MockPath.call_args_list if c != call.home()
        ]
        # Ensure it wasn't called like Path("/some/string") if 'base' was missing
        string_constructor_calls = [
            c for c in path_constructor_calls if isinstance(c.args[0], str)
        ]
        self.assertEqual(
            len(string_constructor_calls),
            0,
            "Path() should not have been called with a string argument in this fallback case.",
        )
...
class TestNumpyEncoder(unittest.TestCase):
    ...
    def test_path(self):
        ...
        data = {"path": p}
        result = json.dumps(data, cls=NumpyEncoder)
        # FIX for Windows path escaping
        loaded = json.loads(result)
        self.assertEqual(loaded["path"], str(p))
...
class TestLogQuery(unittest.TestCase):
    ...
    def test_log_query_full(
        ...
        mock_log_file_instance = MagicMock(spec=Path)
        MockPath.return_value = mock_log_dir_instance
        mock_log_dir_instance.__truediv__.return_value = mock_log_file_instance
        ...
        expected_debug = {
            "retrieval_time": 0.2,
            "extra_debug": "value",
        }  # raw_llm_output excluded by default
    ...
    def test_log_query_file_write_fail(
        ...
        self.mock_logger.error.assert_called_once_with(
            f"Error saving query log to {mock_log_file_instance}: Disk full"
        )
        self.assertEqual(result_path, "")
