...
class TestHeaderComponent(unittest.TestCase):
    ...
    def test_header_component_logo_found(
        self, MockQPixmap, MockQLabelCls, MockQHBoxLayout, MockQWidget, mock_resources, mock_logger
    ):
        mock_logo_resource_path = MagicMock() 
        mock_resources.files.return_value.joinpath.return_value = mock_logo_resource_path
        ...
        mock_resources.files.return_value.joinpath.assert_called_once_with("assets/llamasearch.png")
        ...
        mock_logger.debug.assert_any_call(f"Loaded logo resource: {mock_logo_resource_path}")
    ...
    def test_header_component_logo_not_found_fallback(
        ...
        mock_logo_label.setFixedSize.assert_called_once_with(60, 60)
        mock_logo_label.setAlignment.assert_called_once_with(Qt.AlignmentFlag.AlignCenter)
    ...
    def test_header_component_logo_resource_load_exception(
        ...
        mock_resources.files.side_effect = FileNotFoundError("Mocked resource system error")
        ...
        mock_logo_label.setText.assert_called_once_with("LS")
        mock_logo_label.setFixedSize.assert_called_once_with(60, 60)
        mock_logger.error.assert_any_call("Error loading logo resource: Mocked resource system error", exc_info=True)
