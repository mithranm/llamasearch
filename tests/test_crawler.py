...
class Crawl4AICrawler:
    ...
    def normalize_url(self, url: str) -> str:
        ...
        try:
            ...
            # Add trailing slash for root paths WITHOUT query strings
            if (not parsed.path or parsed.path == "/") and not parsed.query and not normalized.endswith("/"):
                normalized += "/"
            return normalized
        ...
        if self.reverse_lookup_path.exists():
            try:
                ...
                logger.error(
                    ...
                    exc_info=True, # Keep exc_info for better debugging
                )
    ...
    def _score_link_priority(
        ...
        try:
            ...
            if not parsed.scheme or not parsed.netloc: # Check for valid parse
                logger.debug(f"URL '{link_url}' parsed to invalid components (scheme/netloc missing). Assigning lowest priority.")
                return float('inf')
        ...
        # Ensure score is not zero before division to avoid DivByZero for very irrelevant links
    ...
    def is_valid_content_url(self, url: str) -> bool:
        ...
        try:
            ...
            ignore_extensions = (
                r"\.(jpg|jpeg|png|gif|bmp|tif|tiff|ico|webp|svg" # No extra |
                r"|css|js|json|xml" # No extra |
                r"|mp3|mp4|avi|mov|wmv|flv|mkv|webm" # No extra |
                r"|woff|woff2|ttf|eot|otf" # No extra |
                r"|pdf|zip|tar|gz|rar|7z|exe|dmg|iso|bin|msi" # No extra |
                r"|doc|docx|xls|xlsx|ppt|pptx|csv|rtf|odt|ods|odp" # No extra |
