...
from PySide6.QtCore import QObject, Signal  # Import QObject for signals
...
class TestAppLogicModule(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.module_logger_patcher = patch(
            MOCK_APP_LOGIC_LOGGER_INSTANCE_TARGET, spec=logging.Logger
        )
    ...
    def setUp(self):
        ...
        for method_name in ["error", "info", "warning", "debug", "critical"]:
            if hasattr(self.mock_app_logic_module_logger_direct, method_name):
                getattr(
                    self.mock_app_logic_module_logger_direct, method_name
                ).reset_mock()
        ...
        self.MockCrawl4AICrawler = patch(
            "llamasearch.ui.app_logic.Crawl4AICrawler"
        ).start()
    ...
    def test_app_logic_init_llm_search_fail_model_not_found(self):
        ...
        mock_status_slot.assert_called_once_with(
    ...
    def test_run_in_background_success(self):
        ...
        test_completion_signal_emitter.manual_index_completed.connect(
            mock_completion_slot
        )
        completion_signal_instance = (
            test_completion_signal_emitter.manual_index_completed
        )
        ...
        self.app_logic._run_in_background(
            mock_task_func, "arg1", completion_signal=completion_signal_instance
        )
        ...
        if 0 in self.qtimer_lambdas_map and self.qtimer_lambdas_map[0]:
            ...
            for lmbd in self.qtimer_lambdas_map[0]:
                ...
                if mock_reenable_slot.called:
                    ...
                    break
            self.assertTrue(
                found_and_called_reenable, "Re-enable lambda not found or not effective"
            )
    ...
    def test_run_in_background_exception_in_task(self):
        ...
        self.app_logic._run_in_background(
            mock_task_func, completion_signal=test_completion_signal
        )
    ...
    def test_run_in_background_task_cancelled(self):
        ...
        mock_completion_slot.assert_called_once_with(
            "Task cancelled during execution.", False
        )
    ...
    def test_submit_search_success(self):
        ...
        with patch.object(self.app_logic, "_run_in_background") as mock_run_bg:
            ...
            mock_run_bg.assert_called_once_with(
                self.app_logic._execute_search_task,
                "test query",
    ...
    def test_submit_crawl_and_index_success(self):
        ...
        with patch.object(self.app_logic, "_run_in_background") as mock_run_bg:
            self.app_logic.submit_crawl_and_index(
        ...
        mock_status_slot.assert_called_with(
            "Starting crawl & index for 1 URL(s)...", "info"
        )

    def test_execute_search_task_success(self):
        self._create_app_logic()
        self.mock_llmsearch_instance.llm_query.return_value = {
            "formatted_response": "HTML response"
        }
        ...
        self.mock_llmsearch_instance.llm_query.assert_called_once_with(
            "a query", debug_mode=False
        )
    ...
    def test_apply_settings_debug_mode_change(self):
        ...
        mock_settings_applied_slot.assert_called_with(
