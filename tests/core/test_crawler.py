...
from llamasearch.core.crawler import (DEFAULT_FETCH_TIMEOUT_S,
                                      DEFAULT_PAGE_TIMEOUT_MS, Crawl4AICrawler,
                                      fetch_single, sanitize_string)
...
class TestCrawlerModuleStandalone(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        cls.module_logger_patcher = patch(
            MOCK_CRAWLER_LOGGER_INSTANCE_TARGET, spec=logging.Logger
        )
    ...
    def setUp(self):
        ...
        for method_name in ["error", "info", "warning", "debug", "critical", "log"]:
            if hasattr(self.mock_crawler_module_logger_direct, method_name):
                getattr(
                    self.mock_crawler_module_logger_direct, method_name
                ).reset_mock()
        ...
        self.strategy_patcher = patch(
            "llamasearch.core.crawler.AsyncPlaywrightCrawlerStrategy"
        )
    ...
    def test_sanitize_string(self):
        self.assertEqual(
            sanitize_string("http://example.com/path?query=1"),
            "example.com_path_query_1",
        )
        self.assertEqual(
            sanitize_string("https://www.example.com/a b/c:d*e<f>g|h="),
            "example.com_a_b_c_d_e_f_g_h",
        )
        self.assertEqual(sanitize_string("test" * 20, max_length=10), "testtestte")
        self.assertEqual(sanitize_string(""), "default")
        self.assertEqual(sanitize_string("///???==="), "default")
        self.assertEqual(
            sanitize_string("https://example.com/%E4%B8%AD%E6%96%87"),
            "example.com_中文",
        )
    ...
    async def test_fetch_single_success_crawl_result_container(self, mock_wait_for):
        ...
        result = await fetch_single(
            mock_crawler_instance, url, cfg, self.shutdown_event
        )

        self.assertIs(result, mock_crawl_result)
        mock_wait_for.assert_called_once()
        self.assertIs(
            mock_wait_for.call_args[0][0], mock_crawler_instance.arun.return_value
        )
    ...
    async def test_fetch_single_shutdown_before_fetch(self):
        ...
        self.mock_crawler_module_logger_direct.info.assert_called_with(
            f"Shutdown requested before fetching {url}"
        )
    ...
    async def test_fetch_single_timeout(self, mock_wait_for):
        ...
        self.assertIsNone(result)
        self.mock_crawler_module_logger_direct.error.assert_called_with(
            f"Timeout ({DEFAULT_FETCH_TIMEOUT_S}s) fetching {url} with Playwright"
        )
    ...
    async def test_fetch_single_playwright_error(self, mock_wait_for):
        ...
        self.assertIsNone(result)
        self.mock_crawler_module_logger_direct.error.assert_called_with(
            f"Playwright exception fetching {url}: Playwright error", exc_info=True
        )
    ...
    def test_crawler_init_success(self):
        ...
        crawler = Crawl4AICrawler(
            ...
            verbose_logging=self.test_class_verbose_logging,
        )
    ...
    def test_crawler_init_no_root_urls(self):
        with self.assertRaisesRegex(
            ValueError, "At least one root URL must be provided."
        ):
            Crawl4AICrawler(
                [], self.temp_dir, verbose_logging=self.test_class_verbose_logging
            )

    def test_crawler_init_strategy_fail(self):
        self.MockStrategyCls.side_effect = RuntimeError("Strategy init failed")
        with self.assertRaisesRegex(
            RuntimeError, "Could not initialize Playwright crawler strategy."
        ):
            Crawl4AICrawler(
                ["http://example.com"],
                self.temp_dir,
    ...
    def test_normalize_url(self):
        ...
        self.assertEqual(
            crawler.normalize_url("http://example.com/path#frag"),
            "http://example.com/path/",
        )
        self.assertEqual(
            crawler.normalize_url("https://example.com/path?q=1"),
            "https://example.com/path?q=1",
        )
        self.assertEqual(
            crawler.normalize_url("https://example.com//path//"),
            "https://example.com/path/",
        )
        self.assertEqual(crawler.normalize_url("ftp://example.com"), "")
        self.assertEqual(crawler.normalize_url(123), "")  # type: ignore
        self.assertEqual(
            crawler.normalize_url("http://example.com"), "http://example.com/"
        )
        self.assertEqual(
            crawler.normalize_url("http://example.com/file.html"),
            "http://example.com/file.html",
        )
        self.assertEqual(
            crawler.normalize_url("http://example.com/dir"), "http://example.com/dir/"
        )
        self.assertEqual(
            crawler.normalize_url("http://example.com/path?b=2&a=1"),
    ...
    def test_is_valid_content_url(self):
        ...
        crawler = Crawl4AICrawler(
            root_urls, self.temp_dir, verbose_logging=self.test_class_verbose_logging
        )

        self.assertTrue(crawler.is_valid_content_url("http://example.com/page"))
        self.assertTrue(
            crawler.is_valid_content_url("http://sub.example.org/another/page.html")
        )
        self.assertTrue(
            crawler.is_valid_content_url("http://docs.sub.example.org/api/reference")
        )
    ...
    async def test_run_crawl_success_flow(
        self, MockAsyncWebCrawlerCls, mock_fetch_single_func
    ):
        ...
        crawl_res1 = MagicMock(
            spec=CrawlResult, success=True, markdown=result1_markdown, links={}
        )
        ...
        mock_fetch_single_func.assert_called_once_with(
            mock_actual_crawler_instance,
            "http://example.com/",
            crawler._run_cfg,
            crawler._shutdown_event,
    ...
    @patch("llamasearch.core.crawler.fetch_single", new_callable=AsyncMock)
    @patch("llamasearch.core.crawler.AsyncWebCrawler")
    async def test_run_crawl_target_links_reached(
        self, MockAsyncWebCrawlerCls, mock_fetch_single_func
    ):
        mock_actual_crawler_instance = MockAsyncWebCrawlerCls.return_value
        mock_actual_crawler_instance.close = AsyncMock()
        ...
        mock_fetch_single_func.return_value = MagicMock(
            spec=CrawlResult, success=True, markdown="content", links={}
        )
    ...
    @patch("llamasearch.core.crawler.fetch_single", new_callable=AsyncMock)
    @patch("llamasearch.core.crawler.AsyncWebCrawler")
    async def test_run_crawl_max_depth_reached(
        self, MockAsyncWebCrawlerCls, mock_fetch_single_func
    ):
        mock_actual_crawler_instance = MockAsyncWebCrawlerCls.return_value
        mock_actual_crawler_instance.close = AsyncMock()
        ...
        crawl_res_root = MagicMock(
            spec=CrawlResult,
            success=True,
            markdown="root content",
            links={"internal": ["/link1"]},
        )
    ...
    @patch("llamasearch.core.crawler.fetch_single", new_callable=AsyncMock)
    @patch("llamasearch.core.crawler.AsyncWebCrawler")
    async def test_run_crawl_shutdown_event(
        self, MockAsyncWebCrawlerCls, mock_fetch_single_func
    ):
        mock_actual_crawler_instance = MockAsyncWebCrawlerCls.return_value
        mock_actual_crawler_instance.close = AsyncMock()
        ...
        mock_fetch_single_func.side_effect = (
            lambda *args, **kwargs: self.shutdown_event.set() or None
        )

        await crawler.run_crawl()
        self.assertTrue(self.shutdown_event.is_set())
        self.mock_crawler_module_logger_direct.warning.assert_any_call(
            "Crawl loop aborted by signal."
        )

    @patch("llamasearch.core.crawler.fetch_single", new_callable=AsyncMock)
    @patch("llamasearch.core.crawler.AsyncWebCrawler")
    async def test_run_crawl_fetch_error(
        self, MockAsyncWebCrawlerCls, mock_fetch_single_func
    ):
        mock_actual_crawler_instance = MockAsyncWebCrawlerCls.return_value
        mock_actual_crawler_instance.close = AsyncMock()
        ...
        self.mock_crawler_module_logger_direct.warning.assert_any_call(
            "Playwright fetch failed or aborted for http://example.com/"
        )

    @patch("llamasearch.core.crawler.fetch_single", new_callable=AsyncMock)
    @patch("llamasearch.core.crawler.AsyncWebCrawler")
    async def test_run_crawl_error_writing_markdown(
        self, MockAsyncWebCrawlerCls, mock_fetch_single_func
    ):
        mock_actual_crawler_instance = MockAsyncWebCrawlerCls.return_value
        mock_actual_crawler_instance.close = AsyncMock()
        ...
        for call_args in self.mock_crawler_module_logger_direct.error.call_args_list:
            if (
                f"Failed to write MD file {crawler.raw_markdown_dir / (crawler._generate_key('http://example.com/') + '.md')}: Disk full"
                in call_args[0][0]
            ):
                error_found = True
                break
        self.assertTrue(
            error_found, "Expected error log for failing to write MD file not found."
        )
