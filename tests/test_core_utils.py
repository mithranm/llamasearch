from llamasearch.ui.utils import QARecord, save_to_db, load_from_db, delete_all_records, export_to_txt
...
def test_delete_all_records():
    ...
    delete_all_records()
    records = load_from_db()
    assert len(records) == 0, f"Expected 0 records after deletion, found {len(records)}."
