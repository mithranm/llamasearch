...
class VectorDB:
    ...
    def __init__(
        ...
        self.embedder= embedder or Embedder(batch_size=16, max_length=text_embedding_size)
        ...
        logger.info(f"Embeddings at: {self.embeddings_path}, meta at: {self.metadata_path}")
    ...
    def add_document(self, file_path:str)->int:
        ...
        try:
            with tempfile.NamedTemporaryFile(delete=False, suffix=".npy") as tf:
                temp_path=tf.name
                logger.info(f"Created temp embeddings file: {temp_path}")

            batch_idx=0
            batches = list(self.chunker.process_file_in_batches(file_path, self.batch_size))
            
            with tqdm(total=len(batches), desc=f"Processing {os.path.basename(file_path)}", unit="batch") as pbar:
                for batch in batches:
                    ...
                    logger.info(f"Generating embeddings for batch {batch_idx+1} with {len(batch)} chunks")
    ...
    def query(self, query_text:str, n_results:int=5)->Dict[str,Any]:
        ...
        if not self.documents:
            ...
            return {"query": query_text, "ids":[], "documents":[], "metadatas":[], "scores":[], "distances":[]}
    ...
    def _vector_search(self, text:str, n_ret=10)->Dict[str,Any]:
        if not text.strip():
            return {"query": text, "ids":[], "documents":[], "metadatas":[], "scores":[], "distances":[]}
        if len(text)>2000:
            text=text[:2000]
        ...
        for i in range(0,c,bsz):
            ...
            sub_sims= cosine_similarity([q_emb], chunk)[0]
            sims[i:end]=sub_sims
    ...
    def _combine_results(self, vres:Dict[str,Any], bmres:Dict[str,Any], n_ret:int)->List[Dict[str,Any]]:
        ...
        sorted_idx= sorted(combined_scores.keys(), key=lambda k:combined_scores[k], reverse=True)
    ...
    def _expand_name_context(self, combined_list:List[Dict[str,Any]], persons:List[str])->List[Dict[str,Any]]:
        """
        ...
        for src in doc_map:
            doc_map[src].sort(key=lambda x:(x[2].get("batch_idx",0), x[2].get("chunk_idx",0)))
