...
class VectorDB:
    ...
    def __init__(
        ...
        if storage_dir:
            ...
            self.storage_dir = os.path.join(
                project_root, 
                f"vector_db_{'pca' if use_pca else 'no_pca'}"
            )
        ...
        if self.use_pca:
            ...
            if self.pca_reducer.load_model():
                logger.info(f"Loaded existing PCA model with {self.pca_components} components")
        ...
        if self.use_pca:
            ...
            logger.info(f"PCA training data: {'READY' if self.has_pca_training_data else 'NOT READY'}")
    ...
    def _is_document_processed(self, file_path: str) -> bool:
        ...
        for meta in self.document_metadata:
            if meta.get('source') == file_path:
                ...
                if abs(meta.get('mtime', 0) - file_mtime) < 0.001:  # Small epsilon for float comparison
                    return True
                else:
                    # File has been modified, remove old entries
                    self.documents = [doc for i, doc in enumerate(self.documents) 
                                    if self.document_metadata[i].get('source') != file_path]
                    self.document_metadata = [meta for meta in self.document_metadata 
                                           if meta.get('source') != file_path]
                    return False
        return False
    ...
    def _train_or_update_pca(self, temp_embeddings_path=None):
        ...
        existing_embeddings_exist = os.path.exists(self.embeddings_path) and os.path.getsize(self.embeddings_path) > 0
        ...
        if not self.has_pca_training_data and temp_embeddings_path and os.path.exists(temp_embeddings_path):
            ...
            if existing_embeddings_exist:
                ...
                if combined.shape[0] >= self.pca_training_size:
                    logger.info(f"Training PCA model with {combined.shape[0]} embeddings")
                ...
                logger.info(f"Combined embeddings saved, but not enough for PCA training yet ({combined.shape[0]}/{self.pca_training_size})")
                return False
            ...
            # No existing embeddings, just use the new ones
                ...
                logger.info(f"New embeddings saved, but not enough for PCA training yet ({new.shape[0]}/{self.pca_training_size})")
                return False
    ...
    def _merge_embeddings_without_pca(self, temp_path: str):
        ...
        if os.path.exists(self.embeddings_path) and os.path.getsize(self.embeddings_path) > 0:
            try:
                ...
                existing_embeddings = np.ascontiguousarray(existing_embeddings, dtype=np.float32)
            ...
            logger.info("No existing embeddings found. Using new embeddings as initial set.")
            shutil.copy(temp_path, self.embeddings_path)
        
        # Save metadata
        self._save_metadata()
        
        logger.info(f"Successfully merged embeddings into database")
    ...
    def add_document(self, file_path: str, metadata: Optional[Dict[str, Any]] = None) -> int:
        ...
        if self.persist and self._is_document_processed(file_path):
            ...
            chunk_count = sum(1 for meta in self.document_metadata if meta.get('source') == file_path)
            return chunk_count
        ...
        try:
            ...
            for batch_idx, batch in enumerate(self.chunker.process_file_in_batches(file_path, self.batch_size)):
                if not batch:
                    continue
                ...
                logger.info(f"Generating embeddings for batch {batch_idx+1} ({len(embedding_texts)} chunks)")
                ...
                logger.info(f"Processed batch {batch_idx+1}: {len(batch)} chunks (total: {added_chunks})")
    ...
    def _get_dynamic_threshold(self, query_text: str) -> float:
        ...
        # Lower threshold for longer, more specific queries
            ...
            threshold = max(0.15, threshold - 0.05)  # Slightly lower for specific questions
    ...
    def query(self, query_text: str, n_results: int = 5) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.use_pca and self.pca_reducer and self.pca_reducer.is_fitted:
                ...
                query_embedding = self.pca_reducer.transform(query_embedding.reshape(1, -1))[0]
            ...
            for i in range(0, embedding_count, batch_size):
                ...
                batch = np.ascontiguousarray(document_embeddings[i:end], dtype=np.float32)
                ...
                if embedding_count > 1000 and (i + batch_size) % 1000 == 0:
                    logger.info(f"Processed {i + batch_size}/{embedding_count} embeddings")
            ...
            if len(filtered_indices) == 0 and len(query_text.split()) <= 3:
                logger.info(f"No results above threshold {effective_threshold}, trying lower threshold")
                filtered_indices = np.where(similarities >= 0.1)[0]  # Very low threshold for simple queries
            ...
            if len(sorted_indices) > 1:
                # Get top results
                top_n = min(len(sorted_indices), n_results * 2)  # Get twice as many for reranking
                ...
                reranked_indices = [top_indices[i] for i in np.argsort(-np.array(overlap_scores))]
                sorted_indices = np.array(reranked_indices + [idx for idx in sorted_indices if idx not in top_indices])
            
            # Add deduplication
            unique_doc_contents = set()
            unique_doc_sources = {}  # THIS IS THE FIX: Initialize as a dictionary, not a set
            unique_indices = []
            ...
            for idx in sorted_indices:
                ...
                if doc_source not in unique_doc_sources or unique_doc_sources[doc_source] < 2:
                    if content_hash not in unique_doc_contents:
                        # Update source count
                        unique_doc_sources[doc_source] = unique_doc_sources.get(doc_source, 0) + 1
            ...
            # Log search summary
            logger.info(f"Found {len(sorted_indices)} unique results above threshold {effective_threshold}")
    ...
    def get_context_for_query(self, query_text: str, n_results: int = 5, debug_mode: bool = False, return_chunks: bool = False) -> Union[str, Tuple[str, List], Tuple[str, Dict, List]]:
        ...
        for i, (doc, meta, distance, similarity) in enumerate(zip(
            results["documents"], 
            results["metadatas"], 
            results["distances"],
