...
class TeapotAI:
    ...
    def _process_embedding_result(self, result: Any) -> np.ndarray:
        """
        Process the embedding result from the model to extract a usable embedding vector.
        
        Args:
            result: The raw result from the embedding model.
            
        Returns:
            np.ndarray: A numpy array containing the processed embedding.
        """
        # Handle different types of embedding outputs
        if isinstance(result, list):
            if len(result) > 0:
                first_element = result[0]
                if isinstance(first_element, (list, np.ndarray)) and len(first_element) > 0:
                    return np.array(first_element[0])
                return np.array(first_element)
        elif isinstance(result, np.ndarray):
            return result
            
        # If we can't process it properly, return a default embedding
    ...
    def _retrieval(self, query: str, documents: List[str], document_embeddings: np.ndarray) -> List[str]:
        ...
        similarities = cosine_similarity(query_embedding_2d, document_embeddings)[0]
    ...
    def extract(self, class_annotation: Type[BaseModel], query: str = "", context: str = "") -> BaseModel:
        ...
        Args:
            class_annotation (Type[BaseModel]): The Pydantic class to extract fields from.
        ...
        for field_name, field in class_annotation.__fields__.items():
            ...
            if type_annotation is bool:
                ...
                if parsed_result and type_annotation is not None:
                    try:
        ...
        # Fix: Use ** to unpack the output dictionary to the class constructor
