...
class Embedder:
    def __init__(self, model_name: str = DEFAULT_MODEL_NAME, device: str = 'cpu', 
                 max_length: int = 512, batch_size: int = 2):  # Increased max_length to 512
        """
    ...
    def embed_batch(self, strings: list) -> np.ndarray:
        ...
        for i in range(0, len(strings), self.batch_size):
            ...
            logger.info(f"Processing batch {i//self.batch_size + 1}/{(len(strings) + self.batch_size - 1)//self.batch_size} ({len(batch)} strings)")
            ...
            with torch.no_grad():
                batch_embeddings = self.model.encode(truncated_batch, convert_to_numpy=True)
            ...
            if (i + self.batch_size) % (self.batch_size * 5) == 0 or (i + self.batch_size >= len(strings)):
                logger.info(f"Processed {min(i + self.batch_size, len(strings))}/{len(strings)} strings")
