...
class MarkdownChunker:
    ...
    def __init__(
        ...
        self.code_block_pattern = re.compile(r'```(?:[a-zA-Z0-9_+-]+)?\n[\s\S]*?\n```', re.MULTILINE)
        ...
        logger.info(f"Initialized MarkdownChunker with text_embedding_size={text_embedding_size} tokens")
    ...
    def _extract_sections(self, html: str) -> List[Dict[str, Any]]:
        ...
        for i, header in enumerate(headers):
            ...
            for element in elements:
                ...
                if current_chunk_elements and current_chunk_size + element_size > max_chunk_size:
        ...
        for pre in pre_blocks:
            ...
            if code:
                ...
                while element and context_paragraphs_before < self.code_context_window:
                    element = element.previous_sibling
                    if element and (element.name in ['p', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'ul', 'ol', 'table'] or 
    ...
    def _extract_text_chunks(self, html: str) -> List[str]:
        ...
        if not headers:
            ...
            for para in paragraphs:
                ...
                if current_chunk and len(current_chunk) + len(para_text) > self.chunk_size:
    ...
    def _truncate_to_token_limit(self, text: str) -> str:
        ...
        for sentence in sentences:
            ...
            if self._estimate_token_count(result + sentence + " ") > self.text_embedding_size:
                break
            result += sentence + " "
    ...
    def _simple_chunk(self, text: str) -> Generator[Dict[str, Any], None, None]:
        ...
        if len(text) < self.min_chunk_size:
            if text.strip():  # Only if it's not just whitespace
                ...
                logger.info(f"Created chunk {chunk_count}: {len(text)} chars (simple chunking - very short text)")
            return
        ...
        for paragraph in paragraphs:
            ...
            if header_match:
                ...
                if current_chunk:
                    ...
                    if chunk_count <= self.max_chunks:
                        ...
                        logger.info(f"Created chunk {chunk_count}: {len(current_chunk)} chars (simple chunking)")
            ...
            if para_tokens > self.text_embedding_size:
                ...
                for sentence in sentences:
                    ...
                    if temp_tokens + sent_tokens > self.text_embedding_size and temp_chunk:
                        chunk_count += 1
                        if chunk_count <= self.max_chunks:
                            yield {
                                "chunk": temp_chunk,
                            ...
                            logger.info(f"Created chunk {chunk_count}: {len(temp_chunk)} chars (simple chunking - sentence split)")
                ...
                if temp_chunk and temp_chunk != (current_header + "\n\n" if current_header else ""):
                    chunk_count += 1
                    if chunk_count <= self.max_chunks:
                        yield {
                            "chunk": temp_chunk,
                        ...
                        logger.info(f"Created chunk {chunk_count}: {len(temp_chunk)} chars (simple chunking - last sentence)")
            ...
            if chunk_count >= self.max_chunks:
                logger.warning(f"Reached maximum chunk limit ({self.max_chunks}). Document chunking stopped.")
                return
    ...
    def chunk_document(self, text: str) -> Generator[Dict[str, Any], None, None]:
        ...
        logger.info(f"Using semantic chunking with code-text pairing (embedding size: {self.text_embedding_size} tokens)")
            ...
            if semantic_chunks and len(semantic_chunks) > 0:
                ...
                for chunk in semantic_chunks:
                    ...
                    if chunk_count <= self.max_chunks:
                        ...
                        logger.info(f"Created chunk {chunk_count}: {len(chunk['combined'])} chars (type: {chunk['metadata']['type']})")
                        yield chunk_dict
                    else:
                        logger.warning(f"Reached maximum chunk limit ({self.max_chunks}). Document chunking stopped.")
                        return
                ...
                logger.info("No semantic chunks created, falling back to simple chunking")
        ...
        try:
            ...
            for chunk_dict in self.chunk_document(text):
                ...
                if len(batch) >= batch_size:
                    total_batches += 1
                    logger.info(f"Yielding batch {total_batches} with {len(batch)} chunks (total chunks: {total_chunks})")
                    yield batch
                ...
                if total_chunks >= self.max_chunks:
                    logger.warning(f"Reached maximum chunk limit ({self.max_chunks}). Stopping batch processing.")
                    if batch:  # Yield any remaining chunks in the batch
                        total_batches += 1
                        logger.info(f"Yielding final batch {total_batches} with {len(batch)} chunks (total chunks: {total_chunks})")
                        yield batch
                    break
            ...
            logger.info(f"File processing complete: {total_batches} batches, {total_chunks} total chunks")
