...
class OptimizedLLM:
    ...
    def __init__(
        ...
        if not os.path.exists(self.model_path):
            ...
            logger.info("You can download the model using: python setup.py --download-model")
    ...
    def _format_retrieved_chunks(self, chunks) -> str:
        ...
        result = [f"{Fore.CYAN}Retrieved {len(chunks)} relevant chunks:{Style.RESET_ALL}\n"]
        ...
        for i, chunk in enumerate(chunks):
            ...
            result.append(f"{Fore.GREEN}Chunk {i+1} ({chunk_type}) - {similarity:.2f}% match:{Style.RESET_ALL}")
    ...
    def _analyze_query_intent(self, query_text):
        ...
        intent["has_greeting"] = any(re.search(pattern, query_text.lower()) for pattern in greeting_patterns)
        ...
        if not intent["information_request"] and len(query_text.split()) > 3:
            ...
            if intent["has_greeting"] and all(word.lower() in [
                "how", "are", "you", "doing", "today", "hope", "well", "fine", 
                "nice", "meet", "pleasure", "glad", "happy", "see", "chat", "talk"
            ] for word in query_text.split() if word.lower() not in [
    ...
    def query(self, query_text: str, context: str = None, 
              show_retrieved_chunks: bool = True, debug_mode: bool = False) -> Union[str, Tuple[str, Dict]]:
        """
        ...
        if not context and query_intent["requires_rag"]:
            ...
            try:
                ...
                if show_retrieved_chunks and context_chunks:
                    retrieved_chunks_display = self._format_retrieved_chunks(context_chunks)
        ...
        try:
            ...
            if query_intent["has_greeting"] and not query_intent["requires_rag"]:
                ...
                output = llm(
                    ...
                    stop=["<|im_end|>", "<|endoftext|>", "<|im_start|>"],  # Precise stop tokens
                    echo=False
                )
            elif query_intent["has_greeting"] and query_intent["requires_rag"]:
    ...
    def chat(self, conversation_history: List[Dict[str, str]], debug_mode: bool = False,
             show_retrieved_chunks: bool = True) -> Union[str, Tuple[str, Dict]]:
        """
        ...
        return self.query(last_user_message, debug_mode=debug_mode, 
    ...
    def add_document(self, file_path: str) -> int:
        ...
        try:
            ...
            logger.info(f"Added {chunk_count} chunks in {time.time() - start_time:.2f}s")
            return chunk_count
        except Exception as e:
...
def main():
    ...
    parser.add_argument("--hide-chunks", action="store_true",
                        help="Hide retrieved chunks in the output")
    ...
    if args.document:
        if os.path.isfile(args.document):
            try:
                ...
                print(f"{Fore.GREEN}Added {chunk_count} chunks from {args.document}{Style.RESET_ALL}")
            except Exception as e:
                print(f"{Fore.RED}Error adding document: {str(e)}{Style.RESET_ALL}")
        elif os.path.isdir(args.document):
            print(f"{Fore.CYAN}Processing documents from directory: {args.document}{Style.RESET_ALL}")
            ...
            if not markdown_files:
                print(f"{Fore.YELLOW}No markdown or text files found in directory: {args.document}{Style.RESET_ALL}")
            else:
                total_chunks = 0
                start_time = time.time()
                ...
                for file_path in markdown_files:
                    ...
                    try:
                        ...
                        print(f"{Fore.GREEN}Added {chunk_count} chunks from {file_path}{Style.RESET_ALL}")
                    except Exception as e:
                        print(f"{Fore.RED}Error processing file {file_path}: {e}{Style.RESET_ALL}")
                
                print(f"\n{Fore.GREEN}Added {total_chunks} total chunks in {time.time() - start_time:.2f}s{Style.RESET_ALL}")
        else:
            print(f"{Fore.RED}Document path not found: {args.document}{Style.RESET_ALL}")
    ...
    if args.query:
        ...
        if args.debug:
            response, debug_info, retrieved_chunks = llm.query(
                args.query,
                debug_mode=True,
                show_retrieved_chunks=show_chunks
            )
            ...
            print(f"{Fore.YELLOW}Detailed logs saved to the logs directory{Style.RESET_ALL}")
        else:
    ...
    if args.interactive:
        ...
        print(f"{Fore.CYAN}Debug mode: {'ON' if debug_enabled else 'OFF'}{Style.RESET_ALL}")
        print(f"{Fore.CYAN}Show chunks: {'ON' if show_chunks else 'OFF'}{Style.RESET_ALL}")
        print(f"{Fore.YELLOW}Detailed logs are being saved to the logs directory.{Style.RESET_ALL}")
